...
function mtnc_get_logo_box()
{
  ...
  function mtnc_get_content_section()
  {
    ...
    if (!empty($mt_options['heading'])) {
      $out_content .= '<h2 class="clickOpent heading font-center" style="font-weight:' . $font_weight . ';font-style:' . $font_style . '">' . wp_kses_post(stripslashes($mt_options['heading'])) . '</h2>';
    }
